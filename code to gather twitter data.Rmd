...
library(lexicon) 
install_github("cran/DescTools")
library(DescTools)
library(SnowballC)
library(wordcloud)
library(RColorBrewer)
library(jmuOutlier)
install_github("cran/jmuOutlier")
...
politicians_df<-read_csv("politicians.csv")
...
freq <- freq %>% filter(!word %like% "http%")
freq$word <- stemDocument(freq$word)
find_full_form <- function(x){
freq$word[x] <<- abbreviation(freq$word[x], sentiments$word)
}
sapply(1:length(freq$word), FUN = find_full_form)
...
fivepersonality_data <- inner_join(library_fivepersonality, freq_for_trait, by = c("X1" = "word"))
...
imagery_data <- inner_join(key_regressive_imagery_our, freq_for_trait, by = c("regex" = "word"))
imagery_data %>% group_by(thinking) %>% summarise(total_value = sum(value)) %>% ggplot(aes(x=thinking, y=total_value, col = category))+geom_bar()
